...
impl Monkeys {
    ...
    fn next_round(&mut self) {
        ...
        for monkey in 0..self.monkeys.len() {
            ...
            while let Some(mut item) = monkey.items.pop() {
                ...
                item = monkey.operation.perform(item) / 3;
    ...
    fn monkey_business(&mut self, no_of_rounds: u64) -> u64 {
        ...
        let mut inspected_items: Vec<_> = self
            .monkeys
            .iter()
            .map(|x| x.borrow().inspected_items)
            .collect();
            ...
            match line.next() {
                ...
                Some("If true") => {
                    self.monkeys[current_monkey]
                        ...
                        .next()
                        .expect("True result not provided")
                        .split_whitespace()
                        .next_back()
                ...
                Some("If false") => {
                    self.monkeys[current_monkey]
                        .borrow_mut()
                        .test
                        .receiver_if_false = line
                        .next()
                        .expect("False result not provided")
                        .split_whitespace()
                        .next_back()
                        .expect("Case true monkey number not provided")
                        .parse::<usize>()
...
pub fn run() -> String {
    ...
    monkeys.monkey_business(20).to_string()
}
...
mod tests {
    ...
    #[test]
    fn verify() {
        assert_eq!(117624.to_string(), run());
    }
}
