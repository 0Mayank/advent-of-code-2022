...
pub fn run() -> String {
    ...
    let unused_space = total_space - root_dir.borrow_mut().size();
    let mut queue = VecDeque::new();
    let mut res = vec![];
    TreeNode::bfs(Rc::clone(&root_dir), &mut queue, &mut res);
    res.iter()
        .filter(|x| {
        ...
        .fold(0, |acc, x| {
            if acc == 0 {
                return x.borrow().size.unwrap();
            }
            if x.borrow().size.unwrap() < acc {
                return x.borrow().size.unwrap();
            }
            acc
        })
...
mod tests {
    ...
    #[test]
    fn verify() {
        assert_eq!(3696336.to_string(), run());
    }
}
